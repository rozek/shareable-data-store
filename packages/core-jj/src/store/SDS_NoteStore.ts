/*******************************************************************************
*                                                                              *
*       SDS_NoteStore - wraps a json-joy CRDT Model without exposing it        *
*                                                                              *
*******************************************************************************/
...
import { SDS_Error, SDS_Entry, SDS_Note, SDS_Link } from '@rozek/sds-core'
import {
...
import type { SDS_ChangeSet, SDS_SyncCursor } from '@rozek/sds-core'
...
//----------------------------------------------------------------------------//
  ...
  export interface SDS_NoteStoreOptions {
...
/**** constructor — initialize store with model and configuration ****/
  ...
  private constructor (Model_:Model, Options?:SDS_NoteStoreOptions) {
    this.#Model = Model_
...
/**** fromBinary — deserialize store from binary snapshot ****/
  ...
  static fromBinary (Binary:Uint8Array, Options?:SDS_NoteStoreOptions):SDS_NoteStore {
    const Decompressed = gunzipSync(Binary)
    const Model_ = Model.fromBinary(Decompressed)
    return new SDS_NoteStore(Model_, Options)
  }

/**** fromJSON — deserialize store from base64-encoded JSON snapshot ****/

  static fromJSON (JSON_:string, Options?:SDS_NoteStoreOptions):SDS_NoteStore {
...
/**** RootNote / TrashNote / LostAndFoundNote — access special notes ****/
  ...
  get RootNote ():SDS_Note {
    return this.#wrap(RootId) as SDS_Note
  }

  get TrashNote ():SDS_Note {
    return this.#wrap(TrashId) as SDS_Note
  }

  get LostAndFoundNote ():SDS_Note {
    return this.#wrap(LostAndFoundId) as SDS_Note
  }
...
/**** EntryWithId — retrieve entry by id ****/
  ...
  EntryWithId (Id:string):SDS_Entry | undefined {
    ...
    if (EntryData != null) {
      return this.#wrap(Id) as SDS_Entry
    }
    return undefined
  }
...
/**** newNoteAt — create new note in specified location ****/
  ...
  newNoteAt (
    ...
    return this.transact(() => {
      ...
      return this.#wrap(NoteId) as SDS_Note
    })
  }
...
/**** newLinkAt — create new link in specified location ****/
  ...
  newLinkAt (
    ...
    return this.transact(() => {
      ...
      return this.#wrap(LinkId) as SDS_Link
    })
  }
...
/**** deserializeNoteInto — deserialize note from JSON into tree ****/
  ...
  deserializeNoteInto (
    ...
    const addEntry = (Entry:any, OuterIdOrNote:string | SDS_Note):void => {
    ...
    return this.#wrap(RootNewId) as SDS_Note
  }
...
/**** EntryMayBeMovedTo — check if entry can be moved to target ****/
  ...
  EntryMayBeMovedTo (Entry:SDS_Entry, OuterNote:SDS_Note, Index?:number):boolean {
    return this._mayMoveEntryTo(Entry.Id, OuterNote.Id, Index)
  }

/**** moveEntryTo — move entry to new location in tree ****/

  moveEntryTo (Entry:SDS_Entry, OuterNote:SDS_Note, Index?:number):void {
...
/**** purgeEntry — permanently delete entry from trash ****/
  ...
  purgeEntry (Entry:SDS_Entry):void {
    if (this._outerNoteIdOf(Entry.Id) !== TrashId) {
      throw new SDS_Error('purge-not-in-trash', 'only direct children of TrashNote can be purged')
    }
  ...
  purgeExpiredTrashEntries (TrashTTL?:number):number {
    ...
    for (const InnerEntryId of TrashChildren) {
      ...
      if (typeof trashedAt === 'number' && ! (Now-trashedAt<TTL)) {
        try {
          this.purgeEntry(this.#wrap(InnerEntryId) as SDS_Entry)
          purgedCount++
        } catch {
...
//----------------------------------------------------------------------------//
//              Internal helpers — called by SDS_Entry / Note / Link           //
//----------------------------------------------------------------------------//
...
/**** _readValueOf — read entry value ****/
  ...
  async _readValueOf (Id:string):Promise<string | Uint8Array | undefined> {
    ...
    switch (true) {
      ...
      default:
        throw new SDS_Error('not-implemented', 'large value fetching requires a ValueStore')
    }
  }
...
/**** _spliceValueOf — modify literal value in-place ****/
  ...
  _spliceValueOf (Id:string, Index:number, DeleteCount:number, Insertion:string):void {
    const Kind = this._ValueKindOf(Id)
    if (Kind !== 'literal') {
      throw new SDS_Error('change-value-not-literal', 'changeValue only works on notes with ValueKind literal')
    }
    this.transact(() => {
...
/**** _innerEntriesOf — get sorted inner entries ****/
  ...
  _innerEntriesOf (Id:string):SDS_Entry[] {
    return this.#sortedInnerEntriesOf(Id).map(entry => this.#wrap(entry.Id)) as SDS_Entry[]
  }

/**** _outerNoteOf — get outer note ****/

  _outerNoteOf (Id:string):SDS_Note | undefined {
    const outerNoteId = this._outerNoteIdOf(Id)
    return outerNoteId ? (this.#wrap(outerNoteId) as SDS_Note) : undefined
  }
...
/**** _outerNotesOf — get ancestor note chain ****/
  ...
  _outerNotesOf (Id:string):SDS_Note[] {
    const Result: SDS_Note[] = []
    let currentId: string | null = this._outerNoteIdOf(Id)
    while (currentId != null) {
      Result.push(this.#wrap(currentId) as SDS_Note)
...
/**** _TargetOf — get link target note ****/
  ...
  _TargetOf (Id:string):SDS_Note | undefined {
    const EntryData = this.#view().Entries[Id]
    const TargetId = (EntryData as any)?.TargetId
    return TargetId ? (this.#wrap(TargetId) as SDS_Note) : undefined
  }
...
/**** #wrapNote — wrap raw note data in SDS_Note object ****/

  #wrapNote (Id:string):SDS_Note {
    const Cached = this.#WrapperCache.get(Id)
    if (Cached instanceof SDS_Note) {
      return Cached
    }
    const Note = new SDS_Note(this, Id)
...
/**** #requireNoteExists — throw if note doesn't exist ****/
  ...
  #requireNoteExists (NoteId:string):void {
    ...
    if (EntryData == null || (EntryData as any).Kind !== 'note') {
      throw new SDS_Error('invalid-argument', `note '${NoteId}' does not exist`)
    }
  }
