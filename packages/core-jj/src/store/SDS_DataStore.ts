...
/**** private state ****/
  ...
  #PatchErrorHandler: ((Error:unknown) => void) | undefined
...
/**** constructor — initialize store with model and configuration ****/
  ...
  private constructor (Model_:Model, Options?:SDS_DataStoreOptions) {
    ...
    this.#PatchErrorHandler = Options?.onApplyPatchError
...
/**** fromBinary — deserialize store from binary snapshot ****/
  ...
  static fromBinary (Serialisation:Uint8Array, Options?:SDS_DataStoreOptions):SDS_DataStore {
    ...
    // fork() assigns a fresh random session ID so that ops generated by this
    // store never collide (same SID + same clock time) with ops from the peer
    // that produced the snapshot.  without the fork, both stores advance from
    // the same (sid, time) origin, and applying a remote patch can overwrite
    // locally-created nodes whose CRDT timestamps clash with the patch ops.
    const restoredStore             = Model.fromBinary(decompressedSerialisation).fork()
    ...
    try {
      this.transact(() => {
        if (Patch_ instanceof Uint8Array) {
          if (Patch_.byteLength === 0) { return }
          // decode the patch array; fall back to treating the whole binary as
          // a single patch when the array header is absent.
          let PatchBinaries:Uint8Array[]
          try {
          ...
          for (const Binary of PatchBinaries) {
            ...
            try {
              ...
              if (this.#PatchErrorHandler != null) { this.#PatchErrorHandler(Signal) }
            }
          }
        } else {
...
/**** recoverOrphans — move orphaned entries to LostAndFound ****/
  ...
  recoverOrphans ():void {
    this.transact(() => {
      ...
      const allIds     = new Set(Object.keys(allEntries))
      for (const [EntryId, EntryData] of Object.entries(allEntries)) {
        ...
        if (outerItemId && ! allIds.has(outerItemId)) {
          ...
          const newOrderKey = generateKeyBetween(this.#lastOrderKeyOf(LostAndFoundId), null)
          this.#Model.api
            .val(['Entries', EntryId, 'outerPlacement'])
            .set(Schema.con({ outerItemId: LostAndFoundId, OrderKey: newOrderKey }))
        ...
        if ((EntryData as any).Kind === 'link') {
          const TargetId = (EntryData as any).TargetId as string | undefined
          if (TargetId && ! allIds.has(TargetId)) {

            /**** create a placeholder item for a dangling link target ****/

            const newOrderKey = generateKeyBetween(this.#lastOrderKeyOf(LostAndFoundId), null)
            this.#Model.api
              .obj(['Entries'])
              .set({ [TargetId]: Schema.obj({
                Kind:           Schema.con('item'),
                outerPlacement: Schema.val(Schema.con({ outerItemId: LostAndFoundId, OrderKey: newOrderKey })),
                Label:          Schema.val(Schema.str('')),
                Info:           Schema.obj({}),
