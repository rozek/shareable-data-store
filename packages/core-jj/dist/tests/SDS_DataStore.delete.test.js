...
describe('SDS_DataStore — Delete & Purge', () => {
    ...
    it('D-04: deleteEntry(RootItem) throws delete-not-permitted', () => {
        const Store = SDS_DataStore.fromScratch();
        expect(() => Store.deleteEntry(Store.RootItem)).toThrowError(expect.objectContaining({ code: 'delete-not-permitted' }));
    });
    it('D-05: deleteEntry(TrashItem) throws', () => {
        const Store = SDS_DataStore.fromScratch();
        expect(() => Store.deleteEntry(Store.TrashItem)).toThrowError(expect.objectContaining({ code: 'delete-not-permitted' }));
    });
    it('D-06: deleteEntry(LostAndFoundItem) throws', () => {
        const Store = SDS_DataStore.fromScratch();
        expect(() => Store.deleteEntry(Store.LostAndFoundItem)).toThrowError(expect.objectContaining({ code: 'delete-not-permitted' }));
    });
    it('D-07: purgeEntry on data not in TrashItem throws purge-not-in-trash', () => {
        const Store = SDS_DataStore.fromScratch();
        const Item = Store.newItemAt(undefined, Store.RootItem);
        expect(() => Store.purgeEntry(Item)).toThrowError(expect.objectContaining({ code: 'purge-not-in-trash' }));
    });
    ...
    it('D-09: purgeEntry throws purge-protected when data has incoming link from RootItem tree', () => {
        ...
        expect(() => Store.purgeEntry(Item)).toThrowError(expect.objectContaining({ code: 'purge-protected' }));
    ...
    it('D-13: data.purge() throws if data not in Trash', () => {
        const Store = SDS_DataStore.fromScratch();
        const Item = Store.newItemAt(undefined, Store.RootItem);
        expect(() => Item.purge()).toThrowError(expect.objectContaining({ code: 'purge-not-in-trash' }));
    });
    // ---------------------------------------------------------------------------
    ...
    it('D-14: deleteEntry records _trashedAt in Info with a timestamp ≥ the time before the call', () => {
        ...
        Store.deleteEntry(Item);
        const TrashedAt = Item.Info['_trashedAt'];
    ...
    it('D-20: dispose() stops the auto-purge timer', () => {
        ...
        try {
            ...
            Store.purgeExpiredTrashEntries = (...args) => {
                purgeSpy();
                return orig(...args);
            };
...
//# sourceMappingURL=SDS_DataStore.delete.test.js.map