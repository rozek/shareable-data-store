...
describe('SDS_DataStore — Value', () => {
    ...
    it('V-13: changeValue on non-literal throws change-value-not-literal', () => {
        const Store = SDS_DataStore.fromScratch();
        const Item = Store.newItemAt(undefined, Store.RootItem);
        expect(() => Item.changeValue(0, 1, 'x')).toThrowError(expect.objectContaining({ code: 'change-value-not-literal' }));
    });
    ...
    it('V-16: writeValue with wrong-type argument throws SDS_Error invalid-argument', () => {
        const Store = SDS_DataStore.fromScratch();
        const Item = Store.newItemAt(undefined, Store.RootItem);
        expect(() => Item.writeValue(42)).toThrowError(expect.objectContaining({ code: 'invalid-argument' }));
    });
    ...
    it('V-19: changeValue with negative fromIndex throws invalid-argument', () => {
        ...
        const Item = Store.newItemAt(undefined, Store.RootItem);
        Item.writeValue('hello');
        expect(() => Item.changeValue(-1, 2, 'x')).toThrowError(expect.objectContaining({ code: 'invalid-argument' }));
    });
    it('V-20: changeValue with toIndex < fromIndex throws invalid-argument', () => {
        const Store = SDS_DataStore.fromScratch();
        const Item = Store.newItemAt(undefined, Store.RootItem);
        Item.writeValue('hello');
        expect(() => Item.changeValue(3, 2, 'x')).toThrowError(expect.objectContaining({ code: 'invalid-argument' }));
    });
    it('V-21: changeValue with non-string replacement throws invalid-argument', () => {
        const Store = SDS_DataStore.fromScratch();
        const Item = Store.newItemAt(undefined, Store.RootItem);
        Item.writeValue('hello');
        expect(() => Item.changeValue(0, 2, 42)).toThrowError(expect.objectContaining({ code: 'invalid-argument' }));
    });
});
//# sourceMappingURL=SDS_DataStore.value.test.js.map