# Test Cases — `@rozek/sds-mcp-server-jj`
...
| EC-03 | `Label: "Docs"`, `MIMEType: "text/markdown"` | `sds_entry_get` returns `Label: "Docs"`, `MIMEType: "text/markdown"` |
| EC-04 | `Value: "hello"` | `sds_entry_get` returns `Value: "hello"` |
| EC-05 | `ValueBase64` set to base64 of `"world"` | `sds_entry_get` value decodes to `"world"` |
| EC-06 | `File` pointing to temp file with known content | `sds_entry_get` value matches file content |
| EC-07 | `Info: { author: "alice" }` | `sds_entry_get` `Info.author` = `"alice"` |
| EC-08 | `at: 0` on item | item appears at index 0 in container's `entry list` result |
| EC-09 | non-existent `Container` | `isError: true`; message contains "not found" |
| EC-10 | `MIMEType` with `Target` | `isError: true` |
| EC-11 | `Value` and `File` together | `isError: true`; message contains "mutually exclusive" |
| EC-12 | `Container: "root"` alias | item created in root; appears in `sds_entry_list` of root |
...
| EG-04 | `Id: "root"` | entry returned; `Id` equals well-known RootId |
| EG-05 | `Id: "trash"` | entry returned; `Id` equals well-known TrashId; `Kind: "item"` |
...
| EG-09 | `Id: "lostandfound"` (no-hyphen variant) | `isError: false`; returned `Id` equals `"lost-and-found"` result |

## EL — `sds_entry_list`

| # | Description | Expected |
|---|---|---|
| EL-01 | `Id: "root"` | array contains created item IDs; every element has `Id` and `Kind` |
| EL-02 | `recursive: true` | nested items appear in result |
...
| EU-05 | `Info: { b: 2 }` on item with existing `{ a: 1 }` | info contains both `a` and `b` |
| EU-06 | `MIMEType` on link | `isError: true`; message contains "link" |
...
| # | Description | Expected |
|---|---|---|
| EV-01 | move item to a different container | `movedTo` matches target ID; item in target list |
...
| EV-05 | `to: "root"` alias | `isError: false`; `movedTo` equals canonical root UUID |
...
| ED-03 | restore trashed entry | `restoredTo` = root ID; entry in root list |
...
| ED-08 | restore with `to: "root"` alias | `isError: false`; `restoredTo` equals canonical root UUID |
...
|---|---|---|
| TL-01 | empty trash | `[]` |
| TL-02 | after `entry delete` | deleted entry ID in result |
...
| # | Description | Expected |
|---|---|---|
| TX-01 | `TTLms: 3153600000000` (≈ 100 years) | `purged: 0`; `TTLms` echoed; entries remain |
...
| BA-03 | `onError: "continue"` — 2nd fails | `Results` has 3 entries; 2nd `ok: false` with `Error`; 3rd `ok: true` |
...
Server started with `createMCPClientWith({ extraEnv / extraArgs })`. All CF tests call `sds_store_info` without explicit `StoreId` / `PersistenceDir` unless noted.

| # | Description | Expected |
|---|---|---|
| CF-01 | `SDS_STORE_ID` env var set | response `StoreId` equals env var value |
...
| CF-04 | `--store` CLI arg | response `StoreId` equals CLI arg value |
...
| CF-07 | `--store` CLI arg + `SDS_STORE_ID` env var | CLI arg wins; `StoreId` equals CLI arg value |
