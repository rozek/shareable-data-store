...
- `@rozek/sds-core-yjs` — store, notes, links, **Y.js CRDT engine** (concrete implementation)
...
> **Note:** This bundle uses [**Y.js**](https://github.com/yjs/yjs) as its CRDT backend (`@rozek/sds-core-yjs`). If you need a different backend, use `@rozek/sds-browser-bundle-jj` (json-joy) or `@rozek/sds-browser-bundle-loro` (Loro CRDT) instead.
...
`@rozek/sds-browser-bundle-yjs` eliminates that: copy `dist/sds-browser-bundle-yjs.js` (≈ 332 KB raw, ≈ 74 KB gzip) to your own infrastructure and point your application's import map or bundler at it. Every byte of SNS code then comes from a single, auditable file served exclusively from your server — making it straightforward to demonstrate **GDPR / DSGVO compliance**.
...
export default defineConfig({
  resolve: {
    alias: {
      ...
      // Note: @rozek/sds-core-yjs must come before @rozek/sds-core so the
...
| --- | --- |
| `SDS_NoteStore`, `SDS_Entry`, `SDS_Note`, `SDS_Link`, `SDS_Error` | `@rozek/sds-core-yjs` (Y.js backend) |
...
```typescript
import { SDS_NoteStore } from '@rozek/sds-browser-bundle-yjs'
