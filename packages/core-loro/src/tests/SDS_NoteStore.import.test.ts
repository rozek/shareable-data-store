...
describe('SDS_NoteStore — Import', () => {
  ...
  it('I-01: deserializeNoteInto imports note into container', () => {
    ...
    const Store2   = SDS_NoteStore.fromScratch()
    const Imported = Store2.deserializeNoteInto(Note1.asJSON(), Store2.RootNote)

    expect(Imported).toBeInstanceOf(SDS_Note)
  ...
  it('I-04: imported note has same MIME type', () => {
    ...
    const Store2   = SDS_NoteStore.fromScratch()
    const Imported = Store2.deserializeNoteInto(Note1.asJSON(), Store2.RootNote) as SDS_Note
  ...
  it('I-05: nested notes are imported with their structure', () => {
    ...
    const Store2     = SDS_NoteStore.fromScratch()
    const Imported   = Store2.deserializeNoteInto(OuterNote.asJSON(), Store2.RootNote) as SDS_Note
  ...
  it('I-06: deserializeLinkInto imports link into container', () => {
    ...
    const Imported   = Store2.deserializeLinkInto(Link1.asJSON(), Store2.RootNote) as SDS_Link
