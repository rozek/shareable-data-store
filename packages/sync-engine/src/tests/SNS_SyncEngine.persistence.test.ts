/*******************************************************************************
*                                                                              *
*                   SNS_SyncEngine — Persistence Tests                         *
*                                                                              *
*******************************************************************************/
...
describe('SNS_SyncEngine — Persistence', () => {
  ...
  it('SP-01: start() calls loadPatchesSince and applies persisted patches to the store', async () => {
    ...
    const SourceStore   = SNS_NoteStore.fromScratch()
    ...
    const TargetStore = SNS_NoteStore.fromBinary(InitialBinary)
    const Persist     = makeMockPersistence([Patch])

    const Engine = new SNS_SyncEngine(TargetStore, { PersistenceProvider:Persist })
    await Engine.start()
  ...
  it('SP-02: internal store change calls appendPatch with patch bytes and monotone seq number', async () => {
    ...
    const Engine = new SNS_SyncEngine(Store, { PersistenceProvider:Persist })
    await Engine.start()
