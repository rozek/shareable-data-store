...
// Persistence: optional SQLite (SNS_DesktopPersistenceProvider) per store
//
// Environment variables:
//   SNS_JWT_SECRET   — HMAC-SHA256 secret (required)
//   SNS_ISSUER       — JWT iss value to validate (optional)
//   SNS_PORT         — listen port (default 3000)
//   SNS_HOST         — bind host (default 127.0.0.1)
//   SNS_PERSIST_DIR  — directory for SQLite DBs (optional; omit = relay-only)
...
export class LiveStore {
  ...
  #Persistence?: SNS_DesktopPersistenceProvider
...
//----------------------------------------------------------------------------//
  ...
  export interface SNS_ServerOptions {
    JWTSecret:   string
    Issuer?:     string
    Port?:       number
    Host?:       string
...
export function createSNSServer (Options?:Partial<SNS_ServerOptions>) {
  const JWTSecretStr = Options?.JWTSecret  ?? process.env['SNS_JWT_SECRET'] ?? ''
  const Issuer       = Options?.Issuer     ?? process.env['SNS_ISSUER']
  const Port         = Options?.Port       ?? parseInt(process.env['SNS_PORT'] ?? '3000', 10)
  const Host         = Options?.Host       ?? process.env['SNS_HOST'] ?? '127.0.0.1'
  const PersistDir   = Options?.PersistDir ?? process.env['SNS_PERSIST_DIR']
