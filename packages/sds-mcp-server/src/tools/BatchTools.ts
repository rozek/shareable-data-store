...
// MCP tool definition and handler for batch execution: sds_batch
// opens the store once, runs all commands sequentially, closes it
...
export const BatchToolDefs:Tool[] = [
...
/**** sds_batch ****/

  {
    name:        'sds_batch',
    description: (
      'execute multiple operations against one store in a single session — ' +
...
export const BatchToolHandlers:Record<string,BatchToolHandler> = {

  'sds_batch': async (Params) => {
    const Config    = configFrom(Params)
    const StoreId   = Config.StoreId
    const onError   = ((Params['onError'] as string | undefined) ?? 'stop').toLowerCase()
    const Commands  = Params['Commands'] as Array<{ Tool:string; Params:Record<string,unknown> }> | undefined
    ...
    for (const Cmd of Commands) {
      ...
      if (! AllowedBatchTools.has(Cmd.Tool)) {
        throw new MCP_ToolError(
          `tool '${Cmd.Tool}' is not allowed inside sds_batch`
        )
      }
