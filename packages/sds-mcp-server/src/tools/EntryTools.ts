...
// MCP tool definitions and handlers for entry operations:
// sds_entry_create, sds_entry_get, sds_entry_list, sds_entry_update,
// sds_entry_move, sds_entry_delete, sds_entry_restore, sds_entry_purge
...
import {
  loadContext, closeContext, resolveEntryId, readFileSafely,
  type BatchSession,
} from '../StoreAccess.js'
...
export const EntryToolDefs:Tool[] = [
...
/**** sds_entry_create ****/

  {
    ...
    description: (
      'create a new item (default) or link (with Target); ' +
...
export const EntryToolHandlers:Record<string,EntryToolHandler> = {
...
export const EntryBatchStepHandlers:Record<string,EntryBatchStepFn> = {
...
async function coreEntryCreate (
  StoreId:string,
  Config:{ StoreId?:string; PersistenceDir:string },
  Params:Record<string,unknown>,
  OpenStore?:SDS_DataStore
):Promise<object> {
  ...
  if (HasTarget) {
    ...
    if (Params['Value']       != null) { throw new MCP_ToolError('Value cannot be combined with Target') }
    if (Params['ValueBase64'] != null) { throw new MCP_ToolError('ValueBase64 cannot be combined with Target') }
    if (Params['File']        != null) { throw new MCP_ToolError('File cannot be combined with Target') }
  }
  ...
  let ContextToClose:{ Engine:{ stop():Promise<void> } } | undefined

  if (UseOpenStore) {
    Store = OpenStore!
  } else {
    const Ctx = await loadContext(FullConfig as { StoreId:string; PersistenceDir:string }, ! HasTarget)
    Store         = Ctx.Store
    ContextToClose = Ctx
  }

  try {
    const ContainerId = resolveEntryId((Params['Container'] as string | undefined) ?? 'root')
    const Container   = Store.EntryWithId(ContainerId) as SDS_Item | undefined
    if ((Container == null) || (! Container.isItem)) {
      throw new MCP_ToolError(`container '${ContainerId}' not found or is not an item`)
    }

    const AtIndex = Params['at'] != null ? requireNonNegativeInt(Params['at'] as number, 'at') : undefined

    if (HasTarget) {
      const TargetId = resolveEntryId(Params['Target'] as string)
      const Target   = Store.EntryWithId(TargetId) as SDS_Item | undefined
      ...
      if (Params['Label'] != null) { Link.Label = Params['Label'] as string }
      applyInfoParams(Store._InfoProxyOf(Link.Id) as Record<string,unknown>, Params)

      return { Id:Link.Id, created:true, Kind:'link', Target:TargetId }
    } else {
      const MIMEType = (Params['MIMEType'] as string | undefined) ?? 'text/plain'
      const Item     = Store.newItemAt(MIMEType, Container, AtIndex)

      if (Params['Label'] != null) { Item.Label = Params['Label'] as string }
...
function coreEntryGet (
  Store:SDS_DataStore, Params:Record<string,unknown>
):object {
  const RawId = Params['Id'] as string
  if (RawId == null) { throw new MCP_ToolError('Id is required') }

  const Id    = resolveEntryId(RawId)
  const Entry = Store.EntryWithId(Id)
  if (Entry == null) { throw new MCP_ToolError(`entry '${Id}' not found`) }

  const Fields   = (Params['Fields']   as string[] | undefined)?.map((f) => f.toLowerCase())
  const InfoKeys = Params['InfoKeys']  as string[] | undefined
  const ShowAll  = (Fields == null) && (InfoKeys == null)

  const Result:Record<string,unknown> = { Id:Entry.Id }

  const wantField = (Name:string) => ShowAll || (Fields?.includes(Name.toLowerCase()) ?? false)

  if (wantField('Kind'))  { Result['Kind']  = Entry.isItem ? 'item' : 'link' }
  if (wantField('Label')) { Result['Label'] = Entry.Label }

  if (Entry.isItem) {
    const Item = Entry as SDS_Item
    if (wantField('MIMEType')) { Result['MIMEType'] = Item.Type }
    if (wantField('Value'))    { Result['Value']    = Store._currentValueOf(Entry.Id) ?? null }
  }
  if (Entry.isLink) {
    if (wantField('Target')) { Result['Target'] = Store._TargetOf(Entry.Id).Id }
  }

  const InfoProxy = Store._InfoProxyOf(Entry.Id) as Record<string,unknown>
  switch (true) {
    case (InfoKeys != null): {
      const InfoSlice:Record<string,unknown> = {}
      for (const Key of InfoKeys!) { InfoSlice[Key] = InfoProxy[Key] ?? null }
...
function coreEntryList (
  ...
  const Id   = resolveEntryId(RawId)
  const Item = Store.EntryWithId(Id) as SDS_Item | undefined
  ...
  const MaxDepth  = Params['Depth'] != null
    ? requireNonNegativeInt(Params['Depth'] as number, 'Depth')
    : Infinity
  ...
  function walkEntries (ContainerId:string, Depth:number):void {
    for (const Entry of Store._innerEntriesOf(ContainerId)) {
      if (SystemListIds.has(Entry.Id)) { continue }
      const Kind = Entry.isItem ? 'item' : 'link'
      if ((RawOnly == null) || (RawOnly === Kind+'s')) {
        const Obj:Record<string,unknown> = { Id:Entry.Id, Kind }

        if (Fields?.includes('label'))   { Obj['Label'] = Entry.Label }
        if (Entry.isItem) {
          if (Fields?.includes('mimetype')) { Obj['MIMEType'] = Store._TypeOf(Entry.Id) }
          if (Fields?.includes('value'))    { Obj['Value']    = Store._currentValueOf(Entry.Id) ?? null }
        }
...
async function coreEntryUpdate (
  Store:SDS_DataStore, Params:Record<string,unknown>
):Promise<object> {
  const RawId = Params['Id'] as string
  if (RawId == null) { throw new MCP_ToolError('Id is required') }
  ...
  if (Entry.isLink) {
    if (Params['MIMEType']    != null) { throw new MCP_ToolError('MIMEType is not supported for links') }
    if (Params['Value']       != null) { throw new MCP_ToolError('Value is not supported for links') }
    if (Params['ValueBase64'] != null) { throw new MCP_ToolError('ValueBase64 is not supported for links') }
    if (Params['File']        != null) { throw new MCP_ToolError('File is not supported for links') }
  }

  validateValueParams(Params)

  if (Params['Label'] != null) {
    (Entry as SDS_Item).Label = Params['Label'] as string
  }

  if (Entry.isItem) {
    const Item = Entry as SDS_Item
    if (Params['MIMEType'] != null) { Item.Type = Params['MIMEType'] as string }
    await applyValueParams(Store, Item.Id, Params)
  }

  applyInfoParams(Store._InfoProxyOf(Id) as Record<string,unknown>, Params)
...
function coreEntryMove (
  Store:SDS_DataStore, Params:Record<string,unknown>
):object {
  const RawId  = Params['Id'] as string
  const RawTo  = Params['to'] as string
  if (RawId == null) { throw new MCP_ToolError('Id is required') }
  if (RawTo == null) { throw new MCP_ToolError('to is required') }
  ...
  if (! Entry.mayBeMovedTo(Target, AtIndex)) {
    throw new MCP_ToolError(
      `cannot move '${Id}' into its own descendant`
    )
  }
...
function coreEntryRestore (
  Store:SDS_DataStore, Params:Record<string,unknown>
):object {
  const RawId  = Params['Id'] as string
  const RawTo  = (Params['to'] as string | undefined) ?? 'root'
  if (RawId == null) { throw new MCP_ToolError('Id is required') }
...
function requireNonNegativeInt (Value:unknown, ParamName:string):number {
...
function validateValueParams (Params:Record<string,unknown>):void {
  const Count = [
...
async function applyValueParams (
  Store:SDS_DataStore, ItemId:string, Params:Record<string,unknown>
):Promise<void> {
  const Item = Store.EntryWithId(ItemId) as SDS_Item
  switch (true) {
    case (Params['File'] != null): {
      const FileData = await readFileSafely(Params['File'] as string)
    ...
    case (Params['Value'] != null): {
      Item.writeValue(Params['Value'] as string)
      break
    }
  }
}
...
function applyInfoParams (
  InfoProxy:Record<string,unknown>, Params:Record<string,unknown>
):void {
  const InfoMap    = Params['Info']       as Record<string,unknown> | undefined
  const InfoDelete = Params['InfoDelete'] as string[] | undefined
