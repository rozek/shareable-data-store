...
import { LostAndFoundId as x, TrashId as D, RootId as L } from "@rozek/sds-core";
...
class ve {
  ...
  async syncWith(t, n, o = 5e3) {
    const r = E({ PersistenceDir: this.#r }, this.#t);
    await this.#e.stop();
    const s = {
      StoreId: this.#t,
...
async function q(e, t = 5e3) {
  ...
  await _.mkdir(e.PersistenceDir, { recursive: !0 });
  const s = E(e, n), d = new $(s, n), a = await d.loadSnapshot(), c = a != null ? v.fromBinary(a) : v.fromScratch(), l = new me(n), u = new U(c, {
...
function W(e) {
  const t = /* @__PURE__ */ new Set([L, D, x]);
  let n = 0;
  function o(r) {
...
const Le = [
  ...
  sds_store_info: async (e, t) => ({
    ...
    DBPath: E({ PersistenceDir: e.PersistenceDir }, e.StoreId)
  }),
...
async function $e(e) {
  ...
  try {
    ...
    return { Server: t, reachable: !1, Error: r };
  }
}
...
function F(e, t) {
  const n = /* @__PURE__ */ new Set([L, D, x]), r = t.innerEntries;
  if (r != null)
    for (const s of r)
...
function Z(e, t) {
  ...
  const c = t.Fields?.map((h) => h.toLowerCase()), l = t.InfoKeys, u = /* @__PURE__ */ new Set([D, x]), p = [];
  function f(h, b) {
...
], Ze = /* @__PURE__ */ new Set(["read", "write", "admin"]), Ge = /^(\d+)(s|m|h|d)$/, Qe = {
...
function rt() {
  ...
  return e.setRequestHandler(we, async () => ({
    ...
    const { name: n, arguments: o = {} } = t.params, r = nt[n];
    if (r == null)
  ...
  for (let n = 0; n < e.length; n++)
    switch (e[n]) {
      ...
      case "--admin-token":
        ...
        break;
      default:
        e[n]?.startsWith("--") && !ot.has(e[n]) && process.stderr.write(
...
async function st() {
  ...
  e.ServerURL != null && !/^wss?:\/\//.test(e.ServerURL) && (process.stderr.write(
    `sds-mcp-server: invalid '--server' URL '${e.ServerURL}' — must start with 'ws://' or 'wss://'
`
  ), process.exit(2)), ge(e);
  const t = rt(), n = new fe();
  await t.connect(n);
