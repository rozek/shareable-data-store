...
describe('entry create (EC)', () => {
  ...
  beforeAll(async () => {
    DataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sds-ec-'))
  })
  ...
  it('EC-01: create without --target creates an item; kind is "item"', async () => {
    const Create = await runCLI([
      '--store', 'test', '--data-dir', DataDir, 'entry', 'create',
    ])
  ...
  it('EC-12: --value combined with --target exits with UsageError (code 2)', async () => {
    const Result = await runCLI([
      '--store', 'test', '--data-dir', DataDir,
      'entry', 'create', '--target', ItemId, '--value', 'some text',
    ])
    expect(Result.ExitCode).toBe(2)
    expect(Result.Stderr).toMatch(/--value.*link/i)
  })

  it('EC-13: --file combined with --target exits with UsageError (code 2)', async () => {
    const Result = await runCLI([
      '--store', 'test', '--data-dir', DataDir,
      'entry', 'create', '--target', ItemId, '--file', '/tmp/sds-nonexistent.txt',
    ])
    expect(Result.ExitCode).toBe(2)
    expect(Result.Stderr).toMatch(/--file.*link/i)
  })

  it('EC-14: --label at link creation time is visible in entry get', async () => {
    const Create = await runCLI([
      '--store', 'test', '--data-dir', DataDir,
  ...
  it('EC-15: --info at link creation time is visible in entry get', async () => {
    const Create = await runCLI([
      '--store', 'test', '--data-dir', DataDir,
  ...
  it('EC-16: --value and --file together exit with UsageError (code 2)', async () => {
    const Result = await runCLI([
      '--store', 'test', '--data-dir', DataDir,
  ...
  it('EC-18: --at with a negative value exits with UsageError (code 2)', async () => {
    const Result = await runCLI([
      '--store', 'test', '--data-dir', DataDir,
  ...
  it('EC-17: using a link as --container exits with NotFound (code 3)', async () => {
...
describe('entry list (EL)', () => {
  ...
  beforeAll(async () => {
    DataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sds-el-'))
...
describe('entry move / delete / restore / purge (EM)', () => {
  ...
  it('EM-02: moving the root or trash item exits with Forbidden (code 6)', async () => {
    // Test moving root
  ...
  it('EM-15: deleting system entries (root, trash) exits with Forbidden (code 6)', async () => {
    // Test deleting root
...
describe('entry update (EU)', () => {
  ...
  beforeAll(async () => {
    DataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sds-eu-'))
...
describe('duplicate options (DO)', () => {
  ...
  beforeAll(async () => {
    DataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sds-do-'))
  })
