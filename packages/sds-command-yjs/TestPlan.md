# Test Plan — `@rozek/sds-command-yjs`

---

## Goal

Verify that the `sds-yjs` CLI binary — the Y.js-backed wrapper around `@rozek/sds-command` — launches correctly, exposes the expected interface, and exits cleanly under basic failure conditions.
...
`@rozek/sds-command-jj` holds the complete integration test suite for all sub-commands. This package runs only the CLI smoke tests (CL + UE) to confirm that the `sds-yjs` binary itself is correctly assembled and starts as expected with the Y.js backend.
...
| **Transport** | spawns built binary `dist/sds-command-yjs.js` |
...
- **CL-01** — `sds-yjs` with no arguments prints help text to stdout and exits with code 0
...
- **CL-05** — `sds-yjs --version` exits with code 0 and prints a semver version string
