# Test Plan — `@rozek/sds-mcp-server-yjs`

---

## Goal

Verify that the `sds-mcp-server-yjs` binary — the Y.js-backed MCP server — starts correctly, exposes all 20 registered tools, and handles store-level operations with the right JSON responses and error flags.

---

## Scope

This package contains a **store-level smoke test** for the Y.js-backed MCP server binary. Tests connect via MCP `StdioClientTransport` and cover:
...
`@rozek/sds-mcp-server-jj` carries the full functional test suite. This package and its `-loro` sibling run the store-level groups (LT + SI + SP + SY + SD + SE) only, to confirm the binary starts correctly and the Y.js backend is wired up.
...
| --- | --- |
| **Runtime** | Node.js 22+ |
| **Store backend** | Y.js CRDT via `@rozek/sds-core-yjs` + `@rozek/sds-persistence-node` |
| **Test framework** | Vitest 2 |
| **Transport** | MCP SDK `StdioClientTransport` — spawns `dist/sds-mcp-server-yjs.js` |
...
```bash
cd packages/sds-mcp-server-yjs
