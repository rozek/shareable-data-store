# Test Cases — `@rozek/sds-command-loro`

All tests spawn the built `dist/sds-command-loro.js` binary.
...
|---|---|---|
| CL-01 | `sds-loro` with no arguments | prints help text; exits with code 0 |
...
| CL-05 | `sds-loro --version` | exits with code 0; stdout matches semver pattern `\d+\.\d+\.\d+` |
