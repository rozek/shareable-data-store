# @rozek/sds-command-loro

CLI tool for the **shareable-data-store** (SDS) family, using the **Loro** CRDT backend. Exposes every store and entry operation as a one-shot command, an interactive REPL shell, and a batch script runner.
...
| requirement | details |
| --- | --- |
| **Node.js 22.5+** | required to run `sds-loro`. Download from [nodejs.org](https://nodejs.org). |
...
All clients and servers connected to the same relay must use the **same CRDT backend**. Use `@rozek/sds-command-loro` together with `@rozek/sds-websocket-server` and `@rozek/sds-mcp-server-loro`.
...
```bash
npm install -g @rozek/sds-command-loro
```

Or use without installation via npx:

```bash
npx @rozek/sds-command-loro entry get <id>
```
...
```
sds-loro [global-options] <command> [command-options]
sds-loro shell
sds-loro --script <file>
```
...
See `@rozek/sds-command` for full command reference. All commands available in `sds-loro` are identical to those in `sds-command` — the only difference is the CRDT backend used to store and synchronise data.
...
```bash
# create an entry
sds-loro --store notes entry create --label "Draft" --mime text/plain

# show the full tree
sds-loro --store notes tree show

# sync with the server
sds-loro --store notes store sync
```
