...
// Browser-only WebRTC-based SNS_NetworkProvider + SNS_PresenceProvider.
// Uses native RTCPeerConnection + RTCDataChannel.
// Signalling is done via a WebSocket connection to /signal/:storeId.
// Falls back to SNS_WebSocketProvider when WebRTC cannot be established.
//
...
import type { SNS_WebSocketProvider } from '@rozek/sns-network-websocket'

//----------------------------------------------------------------------------//
//                                   Types                                    //
//----------------------------------------------------------------------------//

  export interface SNS_WebRTCProviderOptions {
...
export class SNS_WebRTCProvider
  implements SNS_NetworkProvider, SNS_PresenceProvider
{
  readonly StoreID:string

  #Options: SNS_WebRTCProviderOptions
  #PeerId:  string = crypto.randomUUID()
  #Fallback:SNS_WebSocketProvider | undefined
...
/**** Event Handlers ****/
  ...
  #PresenceHandlers:        Set<(PeerId:string, State:SNS_RemotePresenceState | undefined) => void> = new Set()

/**** Presence Peer Set ****/

  #PeerSet: Map<string, SNS_RemotePresenceState> = new Map()
...
/**** connect ****/
  ...
  async connect (URL:string, Options:SNS_ConnectionOptions):Promise<void> {
// TODO: validate URL
...
/**** #setState — updates the connection state and notifies all registered handlers ****/
  ...
  #setState (State:SNS_ConnectionState):void {
...
/**** #handleSignal — dispatches an incoming signalling message to the appropriate handler ****/
  ...
  async #handleSignal (Msg:SignalMessage, ConnOpts:SNS_ConnectionOptions):Promise<void> {
    switch (Msg.type) {
      case 'hello': {
...
/**** #handleFrame — dispatches a received binary data-channel frame to the appropriate handler ****/
  ...
  #handleFrame (Frame:Uint8Array, _RemotePeerId:string):void {
    ...
    switch (Type) {
      ...
      case 0x04: {
        try {
          const State = JSON.parse(new TextDecoder().decode(Payload)) as SNS_RemotePresenceState
